...
浏览器打开 [http://127.0.0.1:4173](http://127.0.0.1:4173)。服务默认只绑定 `127.0.0.1`，局域网和互联网无法访问本机扫描接口。也可以先打开公开看板；公开看板会自动探测本机 `127.0.0.1:4173`，连接成功后自动扫描、识别当前设备并回填结果。

浏览器不能出于安全原因替网页静默启动电脑上的 `npm start`。公开看板未连接助手时，点击“复制启动命令”运行 `npm install` 和 `npm start`，再点击“重新扫描本机”即可；“打开本机助手”只会打开地址，不会伪装成启动器。

如果直接双击 `public/index.html`，新版会尝试以 `file://` 只读方式连接本机助手并显示设备数据，但不会拿到清理令牌。要启用清理并获得最稳定的自动扫描，请使用 `npm start` 后打开本机地址。

## 给其他用户使用

最安全的方式是让用户下载项目并在自己的电脑运行 `npm start`。这样网页看到的是用户本机数据，检查过程全部留在本机。

如果只部署 `public/` 为静态网站，远程浏览器没有权限直接读取或清理用户磁盘。页面会尝试连接该访客自己电脑上的本地助手；只有访客在本机运行 `npm start` 且助手允许该公开站点来源时，本机扫描和可恢复清理才会启用，否则清理按钮保持禁用。浏览器或网络策略阻止 localhost 连接时，用户仍可生成报告并导入：
...
本地助手的跨来源读取只允许内置的 GitHub Pages / 站点来源；自托管站点可在启动前设置逗号分隔的 `SSD_GUARD_ALLOWED_ORIGINS`，例如 `SSD_GUARD_ALLOWED_ORIGINS=https://dashboard.example.com npm start`。不要使用 `*`，也不要把助手绑定到公网地址。
