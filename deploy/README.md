...
公网只部署 `public/`，不要运行或反向代理 `server.mjs`。服务器上的公网网页不能替访客读取或清理其电脑文件；它会在浏览器中尝试连接访客自己电脑上的 `127.0.0.1:4173` 本地助手，连接成功才自动扫描并启用可恢复清理，否则仍可导入 JSON 报告。
...
然后打开公网看板，页面会自动探测本机助手；也可以运行 `npm run scan --silent > ssd-write-guard-report.json`，在公网网页点击「导入报告」。浏览器不能自动替用户执行 `npm start`，页面提供复制启动命令和打开本机助手地址的按钮。报告不包含文件内容、用户名、主机名或对话内容。
