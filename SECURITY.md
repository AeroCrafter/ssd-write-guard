...
The local server binds to `127.0.0.1` by default. Remote binding is rejected unless `SSD_GUARD_ALLOW_REMOTE=1` is set explicitly. Remote binding is not a supported public deployment mode because `/api/scan` and cleanup endpoints operate on the host filesystem. Read-only cross-origin requests from the published dashboard use an explicit origin allowlist and `Access-Control-Allow-Private-Network`; mutation requests still require the random token and an allowlisted origin. Add a self-hosted origin with `SSD_GUARD_ALLOWED_ORIGINS`, never with `*`.
